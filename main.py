...
from peewee import * # peewee library convention
import readchar
import sys
import formats	
...
sqlite_db = SqliteDatabase('task_logger.db', pragmas={'foreign_keys': 1})
...
class Database:
	...
	def delete_entry(self):
		...
		if not db_emp:
			print("There is no employee in the database system with that data")
	...
	def get_employee_data(self):
		...
		employee_query = formats.name(input("Enter the employee name ").title())
		first_name, last_name = employee_query.split()

		ssn_pattern = re.compile(r'\d{3}-\d{2}-\d{4}')
			
		while True:
			ssn_number = input('Enter {staff_employee}\'s social security number:\n>>> '.format(staff_employee=f'{first_name} {last_name}')).strip()
...
'''
	...
	def search_entries(self):
		...
		while True:
			...
			while True:
				search_input = readchar.readkey() # if the keystroke is a number, coerce the byte object into a str for line 92
	...
	def search_employee(self):
		...
		while True:
			...
			with db:
				...
				print(employee_menu)
				print("Which employee do you want to see? ")
				...
				menu_status = formats.display_tasks(all_emp_tasks)
	...
	def search_dates(self):
		...
		while True:
			...
			try:
				...
				collect_date_range = Task.select().where(Task.task_date >= search_start | Task.task_date <= search_end)
				...
				if not collect_date_range:
					...
					query_again = formats.display_tasks(collect_date_range)
					if not query_again:
						break
					continue
	...
	def search_notes(self):
		...
		while True:
			...
			menu_status = formats.display_tasks(tasks_by_phrase)
